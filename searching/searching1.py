        ...
        for i in range(len(str(os.listdir(os.getcwd() + '/' + str(folder))))):
            ...
            try:
                with open(folder_file, 'r') as file:
                    with connection.cursor() as cursor:
                        try:
                            sql = "CREATE TABLE `crew_db`.`{0}` ( `combos` TEXT NOT NULL ) ENGINE = InnoDB;".format(str(quer_bukv))
                            # sql = "CREATE TABLE '{0}' (combos text);".format(str(quer_bukv))
                            print("CREATE TABLE '{0}' (combos text);".format(str(quer_bukv)))
                            cursor.execute(sql)
                            result = cursor.fetchone()
                            connection.commit()
                        except pymysql.err.OperationalError:
                        ...
                        try:
                            ...
                            print("LOAD DATA INFILE '{0}' IGNORE INTO TABLE {1} (combos);".format(str(folder_file).replace('\\', '/'), str(quer_bukv)))
                            cursor.execute(sql)
                            result = cursor.fetchone()
                            connection.commit()
                        except pymysql.err.OperationalError:
